    ...
    if (success) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center gap-4">
                ...
                <p className="text-slate-400">Funds will be credited to your bank account within 24 hours.</p>
            </div>
        );
    }
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="glass-card flex flex-col gap-6">
                <div className="text-center p-4 bg-white/5 rounded-2xl">
                    ...
                    <h2 className="text-2xl font-bold text-white mt-1">₹{profile?.walletBalance.toLocaleString()}</h2>
                </div>

                <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                    <div className="flex flex-col gap-2">
                        <label className="text-sm font-medium text-slate-400 px-1">Withdrawal Amount (Min ₹300)</label>
            ...
            <div className="glass-card mt-2">
                ...
                <div className="flex flex-col gap-1 text-sm">
                    <div className="flex justify-between"><span className="text-slate-500">Holder:</span> <span className="text-white font-medium">{profile?.bankDetails?.name || '-'}</span></div>
                    <div className="flex justify-between"><span className="text-slate-500">Account:</span> <span className="text-white font-medium">{profile?.bankDetails?.accountNo || '-'}</span></div>
                    <div className="flex justify-between"><span className="text-slate-500">IFSC:</span> <span className="text-white font-medium">{profile?.bankDetails?.ifsc || '-'}</span></div>
                </div>
            </div>
        </div>
    );
